...
#E Reichert 2022
# Calculate the no. of incident infections each day, under each strategy

# Make visual comparisons in cumulative cases averted and IRRs by strategy
# under baseline model parameters

# Used to generate Supplementary Fig. 3 for Resistance-minimizing strategies for introducing 
...
f1 <- ggplot(data = incidence) +
  geom_line(aes(x = time, y = IRR, col = Strategy), size = 1.8) +
  ylab("Incidence Rate Ratio") +
  geom_vline(aes(xintercept = LossBoth, col = Strategy), linetype = "longdash", size = 1) +
...
f2 <- ggplot(data = incidence) +
  geom_line(aes(x = time, y = CasesAverted, col = Strategy), size = 1.8) +
