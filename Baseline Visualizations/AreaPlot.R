...
# E Reichert 2022
# Visualize results from "GCtransmission.Rmd" under baseline model assumptions
# Area plot of resistant profiles over time

# Used to generate Supplementary Fig. 2 for Resistance-minimizing strategies for introducing 
...
ggplot(infections,aes(x=time/365, y=percent*100, fill = factor(ResistState, levels = c("Neither", "A only", "B only", "A and B")))) +
  ...
  xlab("Years") + ylab("% of Gonococcal Infections") + theme(text = element_text(size=15))  +
